...
async function handleCardAction(body: CardActionBody): Promise<void> {
  ...
  if (!operatorOpenId || !messageId) {
    await logCallback(admin, "missing_context", { hasOperator: Boolean(operatorOpenId), hasMessage: Boolean(messageId), body: body.event }, "card.action.trigger", messageId, operatorOpenId)
    return
  }
  ...
  if (mappingError || !mapping?.assignment_id) {
    await logCallback(admin, "no_mapping", { mappingError: mappingError?.message }, "card.action.trigger", messageId, operatorOpenId)
  ...
  try {
    ...
    await logCallback(admin, "confirmed", summary, "card.action.trigger", messageId, operatorOpenId)
    ...
    await logCallback(admin, "error", message, "card.action.trigger", messageId, operatorOpenId)
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await logCallback(admin, "bad_json", "request body is not json")
    return NextResponse.json({ ok: false }, { status: 400 })
  }

  if (typeof body.encrypt === "string" && body.encrypt) {
    if (!getFeishuEncryptKey()) {
      await logCallback(admin, "encrypt_key_missing", "encrypted payload but no key configured")
    ...
    try {
      ...
      await logCallback(admin, "decrypt_failed", String((err as Error)?.message ?? err))
  ...
  if (body.type === "url_verification") {
    await logCallback(admin, "url_verification", body.token ? "token present" : "token absent", "url_verification")
  ...
  if (!tokenMatches(headerToken)) {
    await logCallback(admin, "invalid_token", `event_type=${eventType}`, eventType, body.event?.context?.open_message_id, body.event?.operator?.open_id)
    return NextResponse.json({ ok: false, error: "invalid_token" }, { status: 403 })
  }

  if (!isFeishuConfigured()) {
    await logCallback(admin, "feishu_not_configured", "app credentials missing", eventType)
  ...
  try {
    if (eventType === "card.action.trigger") {
      ...
      await logCallback(admin, "ignored_event", "unhandled event type", eventType, body.event?.context?.open_message_id, body.event?.operator?.open_id)
    }
  } catch (err) {
    await logCallback(admin, "handler_crash", String((err as Error)?.message ?? err), eventType)
