...
- 更新 `components/lead-kanban.tsx`：
  - 在看板本地过滤逻辑中新增“团队范围兜底”规则：当当前用户的 `leadScopeType="team"` 且存在 `currentTeamId` 时，无论负责人筛选条件如何，仅保留 `lead.teamId === currentTeamId` 的线索，确保团队权限账号在“全部负责人”视图下也只能看到本团队线索。
  - 调整负责人筛选下拉文案：在 `leadScopeType="team"` 时将“全部负责人”改为“全部团队成员”，在 `leadScopeType="org/custom"` 时仍显示“全部负责人”，使界面语义与实际权限范围对齐。

**变更原因（对应 PRD/原型）**
- 线索权限 PRD 要求“团队数据权限 = 只能在本团队范围内查看线索”，而实际使用中发现团队权限账号在选择“全部负责人”时感知为“看到了全公司线索”，需要通过前端视图范围收紧和文案修正，消除歧义并为将来可能的权限配置波动提供额外保护层。

**影响范围**
- 前端：仅影响线索看板 `/leads` 页面上的负责人筛选和列表内容，对后端 RLS 逻辑不做任何更改；在现有 RLS 正确的前提下，该改动只会收紧前端展示范围，不会放宽实际可见数据。
- 体验：团队权限用户在“全部团队成员”视图下看到的线索列表，更直观地体现“团队范围”，避免误以为拥有全组织视角。

**回滚方式**
- 如需回滚，可将 `components/lead-kanban.tsx` 中新增的团队兜底过滤条件删除，并恢复负责人下拉中“全部团队成员/全部负责人”的分支文案为统一的“全部负责人”，界面行为将回到仅依赖 RLS 的原始状态。
