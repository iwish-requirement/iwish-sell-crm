...
### global-public-pool-access: 公海池全员可见与认领规则收敛

**变更点**
- 新增 `public.public_pool_secure_view`，仅返回未删除的公海线索，并在视图层保留手机号、微信、预算等字段脱敏，不再继承本人/团队/组织范围。
- 公海池导航和页面访问改为所有已启用成员开放；认领仍只需要 `leads.claim` 权限。
- 公海认领继续受“原负责人再次认领”系统设置控制，并使用不受覆盖权限绕过的 60 条有效客户硬上限。
- 追加 `20260915061000_public_pool_claim_hard_limit.sql`，将认领上限固定为 60，不受配额设置异常值影响。

**验证**
- 已直接应用生产 Supabase 迁移并修复迁移历史；前端公海查询已切换到全局安全视图。

### unified-effective-lead-quota: 统一 60 条有效客户配额口径
...
**验证**
- 已直接应用到生产 Supabase 项目并修复迁移历史；已验证触发器、认领函数、分配函数、转移函数及有效客户判定函数均为新版本。
- 前端已移除重复的公海状态更新调用，应用版本更新为 `2026-09-15-02`，并完成 Cloudflare Worker 部署。
